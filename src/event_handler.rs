...
impl EventHandler {
    ...
    pub fn on_button_press(&mut self, state: &mut state::LabyrinthState, event: &gdk::EventButton) -> bool {
        if let Some(ref mut labyrinth) = state.labyrinth {
            return self.handle_mark_box(labyrinth, event.get_position());
        } 
        return false;
    }
    pub fn on_motion_notify(&mut self, state: &mut state::LabyrinthState, event: &gdk::EventMotion) -> bool {
        if let Some(ref mut labyrinth) = state.labyrinth { 
    ...
    fn print_labyrinth(&self, labyrinth:& labyrinth::Labyrinth, cairo_context: &cairo::Context) {
        ...
        for (index, _) in labyrinth.marked.indexed_iter().filter( |&(_, elem)| { *elem }) {
            ...
            if let Some(rectangle) = labyrinth.box_to_pixel((x_box as i32, y_box as i32)) {
                cairo_context.rectangle(rectangle.x as f64, rectangle.y as f64, rectangle.width as f64, rectangle.height as f64);
                cairo_context.fill();
            }
        }
    }
    fn handle_mark_box(&self, labyrinth: &mut labyrinth::Labyrinth, position : (f64, f64)) -> bool {    
        ...
        if let Some(clicked_box) = clicked_box {
            if let Some(marked) = labyrinth.marked.get_mut(ndarray::IxDyn(&[clicked_box.0 as usize, clicked_box.1 as usize])) {
                if !*marked {
                    *marked = true;
                    return true;
                }
